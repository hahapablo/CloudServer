...
bool is_path_safe(const string &root_dir, const string &test_file) {
  ...
  if (realpath(root_dir.c_str(), rootdir) == nullptr || realpath(test_file.c_str(), testdir) == nullptr) {
    return false;
  }
  ...
  string root_str(rootdir);
  string test_str(testdir);
  root_str += "/";
  test_str += "/";

  string sub = test_str.substr(0, root_str.length());
  // if the rootdir length substring of testdir are the same as rootdir, it's safe
  if (sub.compare(root_str) == 0 ){
    return true;
  }
  return false;
